...
{
    class SemiAutomaticSniperAI : Weapons
    {
        ...
        private const int m_SpreadAngle = 8;
        ...
        public SemiAutomaticSniperAI(AbsEntity owner) : base(owner)
        {
            Owner = owner;
        ...
        public override void MouseDown(Point Target)
        {
            if (m_CanShoot)
            {
                ...
                EntityManager.Instance.ProjectilesListHostile.Add(new ProjectileBullet(TextureManager.TextureBullet, new Vector2(Owner.X, Owner.Y), new Vector2(8, 8), MouseDir * m_BulletSpeed, 30) { Friendly = true });
                JouerSonTir();
            }
